...
class ExperienceReplayMemory:
    ...
    def push(self, *args):
        ...
        s, a, r, s_ = args
        self.memory[int(self.position)] = Transition(s.clone().detach(),
                                                a.reshape(1, -1).clone().detach(),
                                                torch.tensor(r).reshape(1, -1).clone().detach(),
                                                s_.clone().detach() if s_ is not None else None)
...
class PrioritizedReplayMemory:
    def __init__(self, size, alpha=0.6, beta_start=0.4, beta_frames=1e4):
        self.device = DEVICE
        self._storage = []
        self._maxsize = size
        self._next_idx = 0
    ...
    def push(self, *data):
        s, a, r, s_ = data
        data = Transition(s.clone().detach(),
                          a.reshape(1, -1).clone().detach(),
                          torch.tensor(r).reshape(1, -1).clone().detach(),
                          s_.clone().detach() if s_ is not None else None)
