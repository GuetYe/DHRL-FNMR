...
class Train:
    ...
    def compare_test(self,
                ...
                if done:
                    if flag == "ALL":
                        bw, delay, loss, length, alley = self.env.get_route_params(
                            mode='train')  # 获得路径的所以链路bw和,delay和,loss和
                        # 添加到数组中
        ...
        np.save(Path('./data/drl_bw'), episode_bw)
        np.save('./data/drl_delay', episode_delay)
        np.save('./data/drl_loss', episode_loss)
        np.save('./data/drl_length', episode_length)

        np.save(Path('./data/kmb_bw'), kmb_bw)
        np.save('./data/kmb_delay', kmb_delay)
        np.save('./data/kmb_loss', kmb_loss)
        np.save('./data/kmb_length', kmb_length)
    ...
    def plot_compare_figure(self, rl_result, kmb_result, x_label, y_label, title):
        ...
        _path = Path('./images')
        if _path.exists():
            plt.savefig(_path / f'{title}.png', dpi=800, bbox_inches='tight', pad_inches=0)
        else:
    ...
    def plot_compare_figure_subplots(self, rl_bw_result, rl_delay_result, rl_loss_result, kmb_bw_result,
        ...
        _path = Path('./images')
        if _path.exists():
            plt.savefig(_path / f'result.png', dpi=300, bbox_inches='tight', pad_inches=0)
        else:
            _path.mkdir(exist_ok=True)
            plt.savefig(_path / f'result.png', dpi=300, bbox_inches='tight', pad_inches=0)
        if args.show:
    ...
    def plot_line_chart(self, data_list, para_name, title, x_label, y_label, legend_list):
        ...
        plt.savefig(_p / f'{title}.png', dpi=300, bbox_inches='tight', pad_inches=0)
        if args.show:
            plt.show()
        plt.close(fig)
...
def test_chart(pt_file_path):
    ...
    _name = f"[{exp_time}]_test"
    train = Train(Config, MulticastEnv, DQN, MyMulticastNet3, name=_name, mode='eval')
    train.compare_test(pt_file_path, figure=True)
...
if __name__ == '__main__':
    ...
    # parser.add_argument("--pt", default="/home/zcw/DRL-M4MR/saved_agents/policy_net-[20230323110055]_[1.0, 0.1, -0.1, -1].pt")
    #14-nodes
    parser.add_argument("--pt", default="/home/zcw/DRL-M4MR/saved_agents/policy_net-[20220629100456]_[1.0, 0.1, -0.1, -1].pt")
    #10-nodes
    # parser.add_argument("--pt", default="/home/zcw/DRL-M4MR/saved_agents/policy_net-[20231124111457]_[1.0, 0.1, -0.1, -1].pt")
    #21-nodes
    # parser.add_argument("--pt", default="/home/zcw/DRL-M4MR/saved_agents/policy_net-[20231124194212]_[1.0, 0.1, -0.1, -1].pt")
