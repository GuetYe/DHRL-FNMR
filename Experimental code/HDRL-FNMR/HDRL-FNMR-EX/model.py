...
class IntrinsicModel(nn.Module):
    def __init__(self, config_file=Path('./config/model_config.yaml'),
        ...
        self.val = nn.Linear(config['fc1']['in_channels'], 1)
    ...
    def forward(self, x):
        ...
        advantage_function = self.fc1(x3)
        state_value = self.val(x3)

        return state_value + (advantage_function - advantage_function.mean())
