...
def plot_compare_figure(rl_result, kmb_result, x_label, y_label, title, mode='bar'):
    width = 0.18
    ...
    if mode == 'bar':
        ...
        plt.bar(range(len(kmb_result)), rl_result, width, label='DHRL-FNMR')
        plt.bar([x + width for x in range(len(kmb_result))],
                kmb_bw, width, label='KMB_bw')
        plt.bar([x + 2 * width for x in range(len(kmb_result))],
                kmb_delay, width, label='KMB_delay')
        plt.bar([x + 3 * width for x in range(len(kmb_result))],
...
def compare_test(config_file: Path = Path('./config/train_config.yaml')):
    ...
    plot_compare_figure(episode_metrics.bw, kmb_bw, "traffic", "mean bw", "bw")
    plot_compare_figure(episode_metrics.delay, kmb_delay, 
                        "traffic", "mean delay", "delay")
    plot_compare_figure(episode_metrics.loss, kmb_loss,
                        "traffic", "mean loss", "loss")
    plot_compare_figure(episode_metrics.length, kmb_length,
                        "traffic", "mean length", "length")
